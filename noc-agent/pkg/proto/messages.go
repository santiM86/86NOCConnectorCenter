...
// SwitchPortsReport carries the interface table of one or more switches
// collected via SNMP ifTable/ifXTable. Consumed by the backend
// _bridge_switch_ports handler which stores it into `switch_ports`
// (same schema as the legacy /connector/switch-ports REST endpoint).
type SwitchPortsReport struct {
        Switches []SwitchInfo `json:"switches"`
}

// SwitchInfo is one switch and its ports.
type SwitchInfo struct {
        LocalIP string           `json:"local_ip"`
        Ports   []SwitchPortInfo `json:"ports"`
}

// SwitchPortInfo is one physical/logical interface of a switch.
...
type SwitchPortInfo struct {
        ...
        LastChangeS int64  `json:"last_change_s"`
