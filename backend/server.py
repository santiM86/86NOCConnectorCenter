...
from routes.install_setup import router as install_setup_router
...
app.include_router(install_setup_router)     # ZIP setup.exe pre-configurato per nuovo cliente (no PowerShell)
...
from routes.port_memory_api import router as port_memory_api_router
...
app.include_router(path_trace_history_router)
...
async def startup_event():
    ...
    try:
        ...
        await db.fusion_shadow.create_index([("client_id", 1), ("device_ip", 1)], unique=True)
    ...
    try:
        ...
        from services.osint_poller import osint_feeds_tick, osint_exposure_tick, osint_c2_tick, kev_asset_alert_tick
        ...
        from services.vital_freshness import scan_all as vital_freshness_scan
        osint_scheduler.add_job(
            vital_freshness_scan, trigger=_OsTrig(minutes=2), id="vital_freshness_tick",
        ...
        from alert_engine import telegram_quiet_digest_tick as _tg_digest
        async def _tg_digest_job():
            from database import db as _db
            await _tg_digest(_db)
        osint_scheduler.add_job(
        ...
        async def _morning_digest_job():
            from database import db as _db
            await _morning_tick(_db)
        osint_scheduler.add_job(
            _morning_digest_job, trigger=_OsTrig(minutes=1),
