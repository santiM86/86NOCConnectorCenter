...
## 2026-09-08 — Redesign TV + Panoramica (solo dati attuali)
- TV: corpo a 2 colonne → sinistra "PROBLEMI IN CORSO" (ProblemCard compatte: solo vitali down / WAN offline-degradata / backup KO / max 3 alert attivi, niente righe "tutto ok"), destra "AZIENDE OK" lista verde. Rimossi roster completo e feed critici in fondo (duplicato). Rosso solo vitali down.
...
## 2026-09-11 — Evidence Fusion v2 ("registro delle prove") + pagina Certezza diagnosi
- evidence_fusion.py: per un device NON raggiungibile raccoglie PROVE con età/peso da 13 fonti (ping, L2 con freschezza, Datto (+match minuti down), Hyper-V, iLO (cache ilo_status/redfish), porta switch (oper/speed/PoE), memoria porta, orario abituale device, flap porta 1h, UPS, Zyxel Nebula, SNMP, connettore, festività). Le prove votano ipotesi concorrenti (healthy_icmp_filtered, intentional_off, hardware_down, os_hung, link_down, power_outage, monitoring_blind). Confidenza = softmax (k=1.3) sui punteggi, cap per numero fonti indipendenti concordi (1→65, 2→82, 3→92, 4+→97); conflitto (seconda ≥70% della prima) → cap 60 + severità declassata. `missing[]` = cosa manca per il 90% (Datto, iLO, SNMP switch, memoria porta in apprendimento, storico). `coverage()` anche per device UP.
- alert_engine: `_apply_fusion_v2` dopo PASS 2 (solo root_cause singoli in FUSION_REFINABLE, mai quelli topologici). SHADOW sempre → `fusion_shadow` (v1 vs v2, agree, applied). `fusion_v2_enabled` (DEFAULT_CONFIG, False) → v2 sostituisce v1 negli alert (evidence[] salvato nell'alert). Toggle da UI.
- routes/fusion.py: GET /api/fusion/certainty/{cid} (cache 60s), /device/{ip}, /shadow, GET/PUT /config.
...
## 2026-09-11 — VM sotto host in topologia + auto-attivazione Evidence Fusion v2
- topology._apply_vm_parents: VM (managed con hyperv_host_ip, oppure MAC virtuale 00:15:5D/VMware/KVM/VBox/Xen sulla STESSA porta switch di un server managed) → padre = host; edge type "vm" (viola tratteggiato), nodo is_vm/vm_host_ip/virtualization; risposta `vm_links`. server_intelligence._auto_attach_hyperv_vms ora salva anche hyperv_host / hyperv_host_ip (risolto dal hostname dello snapshot).
...
## 2026-09-12 — Base conoscenza AI (RAG leggero) + memoria casi + giorni apprendimento configurabili
- ai_knowledge.py: BUILTIN_KB 25 voci curate (HPE IML/SEL: memoria corr/uncorr, PSU, fan, thermal, drive, cache FBWC, ASR, NIC; iLO self-test/security; SPP baseline; porte: 100M, flap, PoE standby/budget, porte inutilizzate, device sconosciuto, attività notturna; CLI Aruba/Comware/Cisco; Zyxel Nebula; Hyper-V; SO bloccato; Datto). search_kb(): scoring per parole chiave (tag×3, titolo×2, corpo×0.3, vendor +1.5), builtin + db.ai_knowledge. case_memory(): alert risolti con nota sullo stesso device/porta + db.ai_feedback. kb_instructions() appesa ai system prompt; ctx AI (port explain/audit, ilo) include knowledge_base + case_memory; schema JSON con kb_refs.
- routes/ai_knowledge_api.py: GET/POST/PUT/DELETE /api/ai/knowledge, GET /api/ai/knowledge/search, POST/GET /api/ai/feedback, GET /api/ai/cases, GET/PUT /api/ai/port-memory/settings {learning_days 1..60} → alert_engine_config.port_memory_learning_days; port_memory.refresh_learning_days() (cache 60s) usato da record/classify/summary/coverage.
- UI: pages/AiKnowledgePage.js (/ai-knowledge, menu "Base conoscenza AI"): lista/ricerca/aggiungi/elimina, tab Memoria casi con accuratezza AI. components/AiFeedback.js ("Verdetto corretto? Sì/No + nota", mostra Fonti KB) sotto PortAiExplain, SwitchAiAudit, IloAiAnalysis. SwitchPortsPage: "apprendimento N gg ✎" editabile (LearningDaysEditor).
- Test: iteration_153 PASS (13/13 backend + frontend).

## 2026-09-12 — Wizard Nuovo Cliente: lista Zyxel Nebula + precompilazione, agent master & scanner
