...
#
# Usage:
#   curl -fsSL https://network-monitor-hub-2.preview.emergentagent.com/downloads/sync-argus.sh | sudo bash
#
...
PKG_URL="${ARGUS_PKG_URL:-https://network-monitor-hub-2.preview.emergentagent.com/api/admin/argus-deploy-latest.tar.gz}"
...
if [[ -f "$ENV_FILE" ]]; then
    ...
    grep -q '^WIZARD_TEMPLATE_FALLBACK_URL=' "$ENV_FILE"  || echo "WIZARD_TEMPLATE_FALLBACK_URL=https://network-monitor-hub-2.preview.emergentagent.com" >> "$ENV_FILE"
    grep -q '^BINARY_FALLBACK_URL=' "$ENV_FILE"           || echo "BINARY_FALLBACK_URL=https://network-monitor-hub-2.preview.emergentagent.com"         >> "$ENV_FILE"
