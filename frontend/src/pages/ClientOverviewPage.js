  ...
  return (
    <div className="p-4 md:p-5 animate-fade-in" data-testid="client-overview-page">
      ...
      <div className="grid grid-cols-2 md:grid-cols-7 gap-2 mb-4">
        ...
        <StatBox label="WAN" value={wanTargets.length > 0 ? (wanTargets.every(t => ["online", "filtered", "degraded"].includes(t.result?.status)) ? "OK" : "ALERT") : "N/C"} color={wanTargets.length === 0 ? "#555" : (wanTargets.every(t => ["online", "filtered", "degraded"].includes(t.result?.status)) ? "#34C759" : "#FF3B30")} sub={wanTargets[0]?.result?.ping?.latency_ms ? `${wanTargets[0].result.ping.latency_ms}ms` : ""} />
