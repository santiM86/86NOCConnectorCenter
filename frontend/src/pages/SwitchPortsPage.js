...
export default function SwitchPortsPage() {
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      const msg = e?.response?.data?.detail || e?.message || "Errore caricamento";
      setError(msg);
      toast.error(msg);
    } finally {
      setLoading(false);
  ...
  // Stati di caricamento/errore molto visibili (prima erano grigi su nero = "schermata nera" percepita)
  if (loading && !data) {
    return (
  ...
  if (error) {
    return (
      <div className="p-6 md:p-10 max-w-3xl mx-auto" data-testid="switch-ports-error">
        <div className="flex items-center gap-3 mb-4">
          <button onClick={() => navigate(-1)} className="text-[var(--text-muted)] hover:text-[var(--text-primary)] text-lg">←</button>
          <h1 className="text-base md:text-lg font-bold">Dettagli switch · <span className="font-mono text-cyan-300">{deviceIp}</span></h1>
        </div>
        <div className="p-6 rounded-lg border border-rose-500/40 bg-rose-500/10 space-y-3">
          <div className="text-sm font-bold text-rose-200">Errore di caricamento</div>
          <code className="block text-[11px] text-rose-100 bg-rose-500/15 px-3 py-2 rounded font-mono break-all">{String(error)}</code>
          <Button onClick={reload} className="bg-rose-500 hover:bg-rose-600 text-white" data-testid="switch-ports-retry">
            <ArrowsClockwise size={13} className="mr-1.5" /> Riprova
          </Button>
        </div>
      </div>
    );
      ...
      <div className="p-6 md:p-10 max-w-3xl mx-auto" data-testid="switch-ports-empty">
        ...
        <div className="p-6 rounded-lg border border-amber-500/40 bg-amber-500/10 space-y-4">
          ...
          <div className="text-[11px] text-amber-50/90 space-y-1 pl-10">
            <div>Cause piu' comuni:</div>
            <ul className="list-disc pl-5 space-y-0.5 text-[var(--text-secondary)]">
              <li>Il device <strong>non e' marcato come switch</strong> in managed_devices (serve <code>device_type</code> switch/router o profilo SNMP Cisco/HPE/ecc.)</li>
              <li>La <strong>community SNMP</strong> configurata non ha permessi di lettura su 1.3.6.1.2.1.2.2.1 (ifTable)</li>
              <li>Il Connector non ha ancora eseguito il ciclo di polling dettagliato (attendi 2-5 min)</li>
              <li>Il device e' <strong>offline</strong> o non raggiungibile via SNMP</li>
            </ul>
          </div>
        </div>
      </div>
